...
function App() {
  const defaults =
    (typeof window !== "undefined" && window.GRAPH_DEFAULTS_MITIGATION) || {};
  ...
  const suppressTooltipText =
    defaults.suppressTooltipText || "Suppress relation between X and Y.";
  const suppressTooltipLines = wrapTextLines(suppressTooltipText, 5);
  let suppressTooltip = null;
  if (xToYArrow) {
    const tooltipWidth = 250;
    const tooltipHeight = suppressTooltipLines.length * 18 + 14;
    const tooltipCenterX = (xToYArrow.x1 + xToYArrow.x2) / 2;
    const tooltipX = tooltipCenterX - tooltipWidth / 2;
    const tooltipY = xToYArrow.y1 - tooltipHeight - 20;
    suppressTooltip = {
      x: tooltipX,
      y: tooltipY,
      w: tooltipWidth,
      h: tooltipHeight,
      lines: suppressTooltipLines,
    };
  }

  const mitigationTooltipText =
    defaults.mitigationArrowTooltipText ||
    "Mitigating factor influences the suppress relation.";
  const mitigationTooltipLines = wrapTextLines(mitigationTooltipText, 5);
  ...
  if (zLabelToXYArrow) {
    ...
    const tooltipHeight = mitigationTooltipLines.length * 18 + 14;
    const arrowMidY = (zLabelToXYArrow.bodyTop + zLabelToXYArrow.bodyBottom) / 2;
    const tooltipX = zLabelToXYArrow.labelX + 28;
    const tooltipY = arrowMidY - tooltipHeight / 2;
    mitigationTooltip = {
      x: tooltipX,
      y: tooltipY,
      w: tooltipWidth,
      h: tooltipHeight,
      lines: mitigationTooltipLines,
    };
  }
          ...
          <g transform="translate(0,30)">
            ...
            <g className="relation-group">
              <AutoBox
                ...
                rectClassName="relation-box"
                textClassName="relation-box-text"
              />
              {xSize && bigBox && (
                <text
                  x={Math.max(bigBox.x + 12, xRenderCenter - xSize.w / 2 - 12)}
              ...
              {xToYArrow && (
                <g>
                  ...
                  {suppressTooltip && (
                    <g className="relation-tooltip">
                      <rect
                        x={suppressTooltip.x}
                        y={suppressTooltip.y}
                        width={suppressTooltip.w}
                        height={suppressTooltip.h}
                        rx="10"
                        ry="10"
                        fill="#1f1f1f"
                        stroke="#111"
                      />
                      <text
                        x={suppressTooltip.x + 12}
                        y={suppressTooltip.y + 20}
                        fill="#f5f5f5"
                        fontSize="16"
                      >
                      {suppressTooltip.lines.map((line, index) => (
                        <tspan
                          key={index}
                          x={suppressTooltip.x + 12}
                          y={suppressTooltip.y + 22 + index * 18}
                        >
                          {line}
                        </tspan>
                      ))}
            ...
            {zLabelToXYArrow && (
              <g className="relation-group diagram-ca-highlight">
                <path
                  d={zLabelToXYArrow.d}
                  className="relation-hitbox"
                />
                <path
                  ...
                  strokeWidth="2"
                  className="relation-arrow-fill"
                />
                {mitigationTooltip && (
                  <g className="relation-tooltip">
                    <rect
                      x={mitigationTooltip.x}
                      y={mitigationTooltip.y}
                      width={mitigationTooltip.w}
                      height={mitigationTooltip.h}
                      rx="10"
                      ry="10"
                      fill="#1f1f1f"
                      stroke="#111"
                    />
                    <text
                      x={mitigationTooltip.x + 12}
                      y={mitigationTooltip.y + 20}
                      fill="#f5f5f5"
                      fontSize="16"
                    >
                      {mitigationTooltip.lines.map((line, index) => (
                        <tspan
                          key={index}
                          x={mitigationTooltip.x + 12}
                          y={mitigationTooltip.y + 20 + index * 16}
                        >
                          {line}
                        </tspan>
                      ))}
...
ReactDOM.createRoot(document.getElementById("root_mitigation")).render(<App />);
