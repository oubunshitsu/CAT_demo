...
function App() {
  const defaults =
    (typeof window !== "undefined" && window.GRAPH_DEFAULTS_DIF_PER2) || {};
  const polarity = defaults.polarity ?? "green";
  // const zBadgePolarity = defaults.zBadgePolarity ?? "green";
  const zBadgePolarity = defaults.zBadgePolarity

  console.log(zBadgePolarity)
  ...
  let bigBoxZ = null;
  if (zSize) {
    const paddingLeft = 120; // left padding
    const paddingRight = 80; // right padding
  ...
  let zToYArrow = null;
  // Routed Z -> Y arrow removed (keep bold arrow only)

  let xToZArrow = null;
  if (xSize && zSize && bigBoxZ) {
    const arrowGap = 2;
    // Start slightly left of the X box center so the relation label has more room.
    const startX = xRenderCenter - 30;
    const startY = yCenter + xSize.h / 2;
    ...
    const labelX = (startX + endX) / 2 + 6;
    xToZArrow = { startX, startY, turnY, endX, labelX };
  }

  let zLabelToXYArrow = null;
  if (xToYArrow && bigBoxZ) {
    const labelX = (xToYArrow.x1 + xToYArrow.x2) / 2;
    const arrowTipOffset = -42; // negative raises tip (shorter), positive lowers (longer)
      ...
      <div className="diagram-container">
        <svg width="100%" height="100%" viewBox="0 0 800 550">
          <g transform="translate(0,30)">
            ...
            {/* Z -> Y routed arrow removed */}

            {/* Arrow from Z→Y label up to X→Y arrow */}
            {zLabelToXYArrow && (
              <path
                d={zLabelToXYArrow.d}
                fill="rgba(242,167,193,0.7)"
...
ReactDOM.createRoot(document.getElementById("root_dif_per2")).render(<App />);
